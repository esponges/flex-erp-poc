...
const transactionAPI = {
  list: async (params: TransactionListParams = {}): Promise<TransactionWithSKU[]> => {
    ...
    const response = await fetch(
      `http://localhost:8080/api/v1/orgs/1100401179193344001/transactions?${queryParams}`,
      {
        headers: {
  ...
  create: async (data: CreateTransactionRequest): Promise<TransactionWithSKU> => {
    const token = localStorage.getItem('auth_token')
    const response = await fetch(`http://localhost:8080/api/v1/orgs/1100401179193344001/transactions`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json',
  ...
  getSummary: async (params: TransactionListParams = {}): Promise<TransactionSummary[]> => {
    const token = localStorage.getItem('auth_token')
    const queryParams = new URLSearchParams()
    ...
    const response = await fetch(
      `http://localhost:8080/api/v1/orgs/1100401179193344001/transactions/summary?${queryParams}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600">Track inventory in and out transactions</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {summary.map((item) => (
            <div key={item.transaction_type} className="bg-white p-4 rounded-lg border border-gray-200">
              <div className="text-sm font-medium text-gray-500">
                {item.transaction_type === 'in' ? 'Inbound' : 'Outbound'} Transactions
              </div>
              <div className="text-2xl font-bold text-gray-900">{item.total_transactions}</div>
              ...
              <form
                onSubmit={(e) => {
                  ...
                  createMutation.mutate({
                    ...
                    transaction_type: formData.get('transaction_type') as 'in' | 'out',
              ...
              >
                <div>
                  ...
                  >
                    ...
                    {skus.filter(sku => sku.is_active).map((sku) => (
